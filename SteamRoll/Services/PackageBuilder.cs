...
public class PackageBuilder
{
    ...
    public async Task<string> CreatePackageAsync(InstalledGame game, string outputPath, PackageOptions? options = null, CancellationToken ct = default, PackageState? resumeState = null)
    {
        ...
        try
        {
            ...
            if (state.CurrentStep <= PackagingStep.CreatingLauncher)
            {
                ...
                DetectAndScriptDependencies(packageDir);
    ...
    /// </summary>
    private void DetectAndScriptDependencies(string packageDir)
    {
        try
        {
            ...
            if (installers.Count > 0)
            {
                ...
                LogService.Instance.Info($"Created dependency installer script for {installers.Count} items", "PackageBuilder");
            }
        }
        catch (Exception ex)
        {
            LogService.Instance.Warning($"Failed to detect dependencies: {ex.Message}", "PackageBuilder");
        }
    }
